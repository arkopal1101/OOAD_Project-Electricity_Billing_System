...
public class PayBill extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            try{
                Conn c = new Conn();
                c.s.executeQuery("update bill set status = 'Paid' where meter_no = '"+meter+"'");
    ...
    public static PayBill getInstance(String meter){
        if(pb==null) 
            pb=new PayBill(meter);
        return pb;
    }
